...
### Bootstrap de agente Zabbix (host/VM)

El despliegue maestro ejecuta `deploy/install-zabbix-agent.sh` para dejar el host con `zabbix-agent2` configurado en modo produccion (TLS-PSK, `ServerActive`, metadata de autoregistro).

Variables relevantes en `secrets.env`:

- `ZABBIX_AGENT_SERVER`
- `ZABBIX_AGENT_ACTIVE_SERVER`
- `ZABBIX_AGENT_HOSTNAME_SUFFIX`
- `ZABBIX_AGENT_METADATA`
- `ZABBIX_AGENT_TLS_IDENTITY` (opcional)
- `ZABBIX_AGENT_PSK` (opcional)

Para aplicarlo manualmente en una maquina concreta:

```bash
sudo -E bash deploy/install-zabbix-agent.sh
```
...
|----------|-----------|--------|
| Zabbix Server | 172.16.30.20 | Web: http://172.16.30.20 |
| Grafana | 172.16.30.21 | http://172.16.30.21:3000 |
| Wazuh Manager | 172.16.30.30 | https://172.16.30.30:5601 |
...
| WireGuard VPN (en router-fw) | 192.168.1.40:51820 | Acceso admin remoto |
...
4. Acceder a servicios internos:
   - SSH: ssh administrator@172.16.30.10
   - MySQL: mysql -h 172.16.20.20 -u quickstay_app -p
...
```
quickstay/
├── deploy-complete.sh              # ⭐ Script maestro de despliegue
...
docker-compose ps router-fw

# Ver logs
docker-compose logs router-fw

# Estado de peers y handshakes
docker-compose exec router-fw wg show
```

### Nota de arquitectura VPN (actual)
```bash
# WireGuard ahora corre dentro de router-fw.
# El servicio legacy "wireguard-vpn" se retiró para reducir complejidad,
# evitar doble ownership de puerto 51820 y mejorar tiempos de despliegue.
```
...
# Historial de componentes retirados / decisiones de limpieza
cat Documentación/COMPONENTES_RETIRADOS.md
```
...
4. **HAProxy Stats** (172.16.10.10:8404): Load Balancer
