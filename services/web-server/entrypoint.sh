...
start_zabbix_agent() {
  ...
  if [ -z "${ZABBIX_AGENT_SERVER:-}" ]; then
    echo "[WARN] ZABBIX_AGENT_SERVER vacio; se omite agente en web"
    return 0
  fi
  ...
  local meta="${ZABBIX_AGENT_METADATA:-quickstay-web}"
...
EOF
  ...
  zabbix_agentd -c /etc/zabbix/zabbix_agentd.conf || echo "[WARN] no se pudo iniciar zabbix_agentd en web"
}
